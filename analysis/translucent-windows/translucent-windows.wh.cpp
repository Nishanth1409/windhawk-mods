...
{
    // Bail out before any DWM, GDI, UxTheme or system color hook is installed,
    // so unrelated applications keep their own rendering.
    if (!IsAllowedProcess()) {
        Wh_Log(L"Unsupported process, mod not loaded");
        return FALSE;
    }
